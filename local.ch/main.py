...
file_handler = logging.FileHandler('scraper_log.log', encoding='utf-8')  # File handler
...
INDEX_TRACK_FILE = 'index_tracker.json'
MAX_RETRIES = 3
RETRY_DELAY = 2
...
def load_last_processed_index():
    try:
        with open(INDEX_TRACK_FILE, 'r') as file:
            ...
            return index_data.get("last_processed_index", 0)
    except (FileNotFoundError, json.JSONDecodeError):
        return 0

# Save the last processed index
def save_last_processed_index(index):
    with open(INDEX_TRACK_FILE, 'w') as file:
